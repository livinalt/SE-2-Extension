// SPDX-License-Identifier: UNLICENSED
    ...
    contract Lending is Ownable {
        ...
        uint256 private constant COLLATERAL_RATIO = 120; // collaterisation ration of 120%
        ...
        function calculateCollateralValue(address user) public view returns (uint256) {
            ...
            uint256 collateralValueInEth = s_userCollateral[user];
            uint256 priceInCorn = i_cornDEX.currentPrice();

            return (collateralValueInEth * priceInCorn) / 1e18;
        }
        ...
        function _calculatePositionRatio(address user) internal view returns (uint256) {
            ...
            uint256 amountBorrowed = s_userBorrowed[user];
            ...
            return (collateralValueInCorn * 1e18) / amountBorrowed;
        }
        ...
        function liquidate(address user) public {
            ...
            // Transfer and burn the user's debt in CORN
            bool success = i_corn.transferFrom(msg.sender, address(this), userDebt);
            
            if (!success) {
                revert Lending__TransferFailed();
            }
            success = i_corn.burnFrom(address(this), userDebt);
            ...
            (success, ) = msg.sender.call{value: amountForLiquidator}("");
        ...
        /**
         * @notice Allows flash loans of CORN that must be repaid in the same transaction
         * @param _recipient The contract receiving the flash loan
         * @param _amount The amount of CORN to loan
         * @param _extraParam Additional parameter to pass to the recipient
         */
        function flashLoan(IFlashLoanRecipient _recipient, uint256 _amount, address _extraParam) public {
            ...
            // Mint CORN to recipient
            bool success = i_corn.mintTo(address(_recipient), _amount);
            if (!success) {
                revert Lending__BorrowingFailed();
            }

            // Call receiver's executeOperation
            success = _recipient.executeOperation(_amount, msg.sender, _extraParam);
            ...
            // Burn returned CORN
            success = i_corn.burnFrom(address(this), _amount);
        ...
        /**
         * @notice Calculates the maximum amount of CORN that can be borrowed for a given ETH collateral
         * @param ethAmount The amount of ETH to deposit as collateral
         * @return uint256 The maximum CORN that can be borrowed
         */
        function getMaxBorrowAmount(uint256 ethAmount) public view returns (uint256) {
            ...
            uint256 collateralValueInCorn = (ethAmount * i_cornDEX.currentPrice()) / 1e18;

            return (collateralValueInCorn * 100) / COLLATERAL_RATIO;
        }

        /**
         * @notice Calculates the maximum amount of ETH collateral that can be withdrawn without liquidation
         * @param user The address of the user to query
         * @return uint256 The maximum ETH that can be withdrawn
         */
        function getMaxWithdrawableCollateral(address user) public view returns (uint256) {
            ...
            uint256 minCollateralValueInCorn = (borrowedAmount * COLLATERAL_RATIO) / 100;

            if (collateralValueInCorn <= minCollateralValueInCorn) {
                return 0;
            }

            uint256 excessCollateralValueInCorn = collateralValueInCorn - minCollateralValueInCorn;
            uint256 priceInCorn = i_cornDEX.currentPrice();

            return (excessCollateralValueInCorn * 1e18) / priceInCorn;
        }
    }